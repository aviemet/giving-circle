  ...
  class Cache
    ...
    class << self
      ...
      def schedule_refresh(presentation_id)
        ...
        PresentationValues::RefreshJob.set(wait: delay).perform_later(presentation_id)
      end
      ...
      def reschedule_if_too_soon(presentation_id, dirty_at_start)
        ...
        delay = [delay, 0].max
        PresentationValues::RefreshJob.set(wait: delay.seconds).perform_later(presentation_id)
      end

      def snapshot_key(presentation_id)
        "presentation_values:#{presentation_id}"
      end

      def dirty_at_key(presentation_id)
        "presentation_values:#{presentation_id}:meta:dirty_at"
      end

      def first_dirty_at_key(presentation_id)
        "presentation_values:#{presentation_id}:meta:first_dirty_at"
      end

      def lock_key(presentation_id)
        "presentation_values:#{presentation_id}:meta:lock"
      end
