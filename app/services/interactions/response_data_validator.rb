class Interactions::ResponseDataValidator
  ...
  def validate_org_money_map(value, prefix)
    ...
    output_metric = orchestration.output_metric
    if output_metric.blank?
      output_metric = legacy_output_metric_from_slug
    end
  ...
  def orchestration
    @orchestration ||= Interactions::Orchestration.parse(@interaction.config)
  end

  def composed_snapshot_for_validation
    presentation = @interaction.presentation
    return {} if presentation.blank?

    PresentationValues::Composer.enrich(
      presentation,
      PresentationValues::Aggregator.new(presentation).raw_snapshot,
  ...
  def validate_pledges_map(value, prefix)
    ...
    funding_by_org = current_funding_cents_by_org
    ...
    value.each_with_index do |entry, index|
      ...
      projected = funding_by_org.fetch(org_id, 0) + amount_cents
  ...
  def allowed_pledge_org_ids(presentation)
    snapshot = composed_snapshot_for_validation
  ...
  def current_funding_cents_by_org
    (composed_snapshot_for_validation[:funding_totals] || []).to_h do |entry|
      [entry[:org_id], entry[:funding_cents].to_i]
    end
  end

  def legacy_output_metric_from_slug
