class Interactions::ConfigValidator
  ...
  def validate
    ...
    if config.blank?
      @record.errors.add(:config, :blank)
      return
    end
    ...
    @errors.each { |message| @record.errors.add(:config, message) }
  end
  ...
  def validate_field(field, path)
    ...
    unless Interactions::Registry.known_type?(type)
  ...
  def validate_output(output, path, field_index)
    ...
    elsif !Interactions::Registry.known_metric?(metric)
    ...
    elsif !Interactions::Registry.known_reducer?(reducer)
      @errors << t(:unknown_reducer, path: path, reducer: reducer)
    elsif !Interactions::Registry.reducer_compatible_with_field?(reducer, field_type)
