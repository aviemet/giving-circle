import { type Data, type DefaultComponents } from "@puckeditor/core"
import { isEqual } from "es-toolkit/compat"

import { type PuckComponentProps } from "../../components"
import { type SlideRootProps } from "../SlideRoot"

export type PuckSlideData = Partial<Data<PuckComponentProps | DefaultComponents, Partial<SlideRootProps>>>

export const EDITOR_LOAD_SOURCE = {
	server: "server",
	localDraft: "localDraft",
} as const

export type EditorLoadSource = (typeof EDITOR_LOAD_SOURCE)[keyof typeof EDITOR_LOAD_SOURCE]

export const EDITOR_SAVE_STATUS = {
	saved: "saved",
	unsaved: "unsaved",
	recovered: "recovered",
} as const

export type EditorSaveStatus = (typeof EDITOR_SAVE_STATUS)[keyof typeof EDITOR_SAVE_STATUS]
...
export function nextEditorChangeState(params: {
	...
	if(params.adoptResolvedBaseline) {
		return {
			...
			saveStatus: EDITOR_SAVE_STATUS.saved,
	...
	return {
		saved: params.saved,
		saveStatus: dirty ? EDITOR_SAVE_STATUS.unsaved : EDITOR_SAVE_STATUS.saved,
...
export function resolveInitialEditorData(params: {
	...
	if(!draft) {
		return {
			data: saved,
			loadSource: EDITOR_LOAD_SOURCE.server,
			saveStatus: EDITOR_SAVE_STATUS.saved,
	...
	return {
		data: draft.data,
		loadSource: EDITOR_LOAD_SOURCE.localDraft,
		saveStatus: EDITOR_SAVE_STATUS.recovered,
		serverFingerprint: savedFingerprint,
	}
}
