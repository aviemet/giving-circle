...
RSpec.describe Interactions::ContextLoader do
  ...
  it "filters pledges orgs by finalists unless allow_non_finalists" do
    ...
    aggregator = instance_double(PresentationValues::Aggregator)
    allow(PresentationValues::Aggregator).to receive(:new).and_return(aggregator)
    allow(aggregator).to receive(:raw_snapshot).and_return(
      {
        allocated_totals: [],
        pledge_totals: [],
        org_vote_totals: [],
