# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `delayed_job` gem.
# Please instead update this file by running `bin/tapioca gem delayed_job`.

# source://delayed_job//lib/delayed/serialization/active_record.rb#2
module ActiveRecord
  class << self
    # source://activerecord/7.1.2/lib/active_record.rb#342
    def action_on_strict_loading_violation; end

    # source://activerecord/7.1.2/lib/active_record.rb#342
    def action_on_strict_loading_violation=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#422
    def allow_deprecated_singular_associations_name; end

    # source://activerecord/7.1.2/lib/active_record.rb#422
    def allow_deprecated_singular_associations_name=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#335
    def application_record_class; end

    # source://activerecord/7.1.2/lib/active_record.rb#335
    def application_record_class=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#263
    def async_query_executor; end

    # source://activerecord/7.1.2/lib/active_record.rb#263
    def async_query_executor=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#317
    def before_committed_on_all_records; end

    # source://activerecord/7.1.2/lib/active_record.rb#317
    def before_committed_on_all_records=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#314
    def belongs_to_required_validates_foreign_key; end

    # source://activerecord/7.1.2/lib/active_record.rb#314
    def belongs_to_required_validates_foreign_key=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#323
    def commit_transaction_on_non_local_return; end

    # source://activerecord/7.1.2/lib/active_record.rb#323
    def commit_transaction_on_non_local_return=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#209
    def db_warnings_action; end

    # source://activerecord/7.1.2/lib/active_record.rb#211
    def db_warnings_action=(action); end

    # source://activerecord/7.1.2/lib/active_record.rb#236
    def db_warnings_ignore; end

    # source://activerecord/7.1.2/lib/active_record.rb#236
    def db_warnings_ignore=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#192
    def default_timezone; end

    # source://activerecord/7.1.2/lib/active_record.rb#196
    def default_timezone=(default_timezone); end

    # source://activerecord/7.1.2/lib/active_record/deprecator.rb#4
    def deprecator; end

    # source://activerecord/7.1.2/lib/active_record.rb#177
    def disable_prepared_statements; end

    # source://activerecord/7.1.2/lib/active_record.rb#177
    def disable_prepared_statements=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#476
    def disconnect_all!; end

    # source://activerecord/7.1.2/lib/active_record.rb#382
    def dump_schema_after_migration; end

    # source://activerecord/7.1.2/lib/active_record.rb#382
    def dump_schema_after_migration=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#392
    def dump_schemas; end

    # source://activerecord/7.1.2/lib/active_record.rb#392
    def dump_schemas=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#465
    def eager_load!; end

    # source://activerecord/7.1.2/lib/active_record.rb#361
    def error_on_ignored_order; end

    # source://activerecord/7.1.2/lib/active_record.rb#361
    def error_on_ignored_order=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record/gem_version.rb#5
    def gem_version; end

    # source://activerecord/7.1.2/lib/active_record.rb#454
    def generate_secure_token_on; end

    # source://activerecord/7.1.2/lib/active_record.rb#454
    def generate_secure_token_on=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#286
    def global_executor_concurrency; end

    # source://activerecord/7.1.2/lib/active_record.rb#278
    def global_executor_concurrency=(global_executor_concurrency); end

    # source://activerecord/7.1.2/lib/active_record.rb#266
    def global_thread_pool_async_query_executor; end

    # source://activerecord/7.1.2/lib/active_record.rb#290
    def index_nested_attribute_errors; end

    # source://activerecord/7.1.2/lib/active_record.rb#290
    def index_nested_attribute_errors=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#183
    def lazily_load_schema_cache; end

    # source://activerecord/7.1.2/lib/active_record.rb#183
    def lazily_load_schema_cache=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#245
    def legacy_connection_handling=(_); end

    # source://activerecord/7.1.2/lib/active_record.rb#308
    def maintain_test_schema; end

    # source://activerecord/7.1.2/lib/active_record.rb#308
    def maintain_test_schema=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#457
    def marshalling_format_version; end

    # source://activerecord/7.1.2/lib/active_record.rb#461
    def marshalling_format_version=(value); end

    # source://activerecord/7.1.2/lib/active_record.rb#373
    def migration_strategy; end

    # source://activerecord/7.1.2/lib/active_record.rb#373
    def migration_strategy=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#425
    def query_transformers; end

    # source://activerecord/7.1.2/lib/active_record.rb#425
    def query_transformers=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#305
    def queues; end

    # source://activerecord/7.1.2/lib/active_record.rb#305
    def queues=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#440
    def raise_int_wider_than_64bit; end

    # source://activerecord/7.1.2/lib/active_record.rb#440
    def raise_int_wider_than_64bit=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#311
    def raise_on_assign_to_attr_readonly; end

    # source://activerecord/7.1.2/lib/active_record.rb#311
    def raise_on_assign_to_attr_readonly=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#242
    def reading_role; end

    # source://activerecord/7.1.2/lib/active_record.rb#242
    def reading_role=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#320
    def run_after_transaction_callbacks_in_order_defined; end

    # source://activerecord/7.1.2/lib/active_record.rb#320
    def run_after_transaction_callbacks_in_order_defined=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#189
    def schema_cache_ignored_tables; end

    # source://activerecord/7.1.2/lib/active_record.rb#189
    def schema_cache_ignored_tables=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#353
    def schema_format; end

    # source://activerecord/7.1.2/lib/active_record.rb#353
    def schema_format=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#395
    def suppress_multiple_database_warning; end

    # source://activerecord/7.1.2/lib/active_record.rb#402
    def suppress_multiple_database_warning=(value); end

    # source://activerecord/7.1.2/lib/active_record.rb#367
    def timestamped_migrations; end

    # source://activerecord/7.1.2/lib/active_record.rb#367
    def timestamped_migrations=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#432
    def use_yaml_unsafe_load; end

    # source://activerecord/7.1.2/lib/active_record.rb#432
    def use_yaml_unsafe_load=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#298
    def verbose_query_logs; end

    # source://activerecord/7.1.2/lib/active_record.rb#298
    def verbose_query_logs=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#415
    def verify_foreign_keys_for_fixtures; end

    # source://activerecord/7.1.2/lib/active_record.rb#415
    def verify_foreign_keys_for_fixtures=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record/version.rb#7
    def version; end

    # source://activerecord/7.1.2/lib/active_record.rb#332
    def warn_on_records_fetched_greater_than; end

    # source://activerecord/7.1.2/lib/active_record.rb#332
    def warn_on_records_fetched_greater_than=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#239
    def writing_role; end

    # source://activerecord/7.1.2/lib/active_record.rb#239
    def writing_role=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record.rb#447
    def yaml_column_permitted_classes; end

    # source://activerecord/7.1.2/lib/active_record.rb#447
    def yaml_column_permitted_classes=(_arg0); end
  end
end

# source://delayed_job//lib/delayed/serialization/active_record.rb#3
class ActiveRecord::Base
  include ::ActiveModel::Access
  include ::ActiveModel::ForbiddenAttributesProtection
  include ::ActiveModel::AttributeAssignment
  include ::ActiveModel::Serialization
  include ::JsonbAccessor::Macro

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#70
  def __callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#70
  def __callbacks?; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _before_commit_callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _commit_callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _create_callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _destroy_callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _find_callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _initialize_callbacks; end

  # source://activerecord/7.1.2/lib/active_record/reflection.rb#11
  def _reflections; end

  # source://activerecord/7.1.2/lib/active_record/reflection.rb#11
  def _reflections?; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _rollback_callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_before_commit_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_commit_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_create_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_destroy_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_find_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_initialize_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_rollback_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_save_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_touch_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_update_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_validate_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#951
  def _run_validation_callbacks(&block); end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _save_callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _touch_callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _update_callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _validate_callbacks; end

  # source://activesupport/7.1.2/lib/active_support/callbacks.rb#963
  def _validation_callbacks; end

  # source://activemodel/7.1.2/lib/active_model/validations.rb#71
  def _validators; end

  # source://activemodel/7.1.2/lib/active_model/validations.rb#71
  def _validators?; end

  # source://activerecord/7.1.2/lib/active_record/reflection.rb#12
  def aggregate_reflections; end

  # source://activerecord/7.1.2/lib/active_record/reflection.rb#12
  def aggregate_reflections?; end

  # source://activestorage/7.1.2/lib/active_storage/reflection.rb#53
  def attachment_reflections; end

  # source://activestorage/7.1.2/lib/active_storage/reflection.rb#53
  def attachment_reflections?; end

  # source://activemodel/7.1.2/lib/active_model/attribute_methods.rb#72
  def attribute_aliases; end

  # source://activemodel/7.1.2/lib/active_model/attribute_methods.rb#72
  def attribute_aliases?; end

  # source://activemodel/7.1.2/lib/active_model/attribute_methods.rb#73
  def attribute_method_patterns; end

  # source://activemodel/7.1.2/lib/active_model/attribute_methods.rb#73
  def attribute_method_patterns?; end

  # source://activerecord/7.1.2/lib/active_record/reflection.rb#13
  def automatic_scope_inversing; end

  # source://activerecord/7.1.2/lib/active_record/reflection.rb#13
  def automatic_scope_inversing?; end

  # source://activerecord/7.1.2/lib/active_record/integration.rb#16
  def cache_timestamp_format; end

  # source://activerecord/7.1.2/lib/active_record/integration.rb#16
  def cache_timestamp_format?; end

  # source://activerecord/7.1.2/lib/active_record/integration.rb#24
  def cache_versioning; end

  # source://activerecord/7.1.2/lib/active_record/integration.rb#24
  def cache_versioning?; end

  # source://activerecord/7.1.2/lib/active_record/integration.rb#32
  def collection_cache_versioning; end

  # source://activerecord/7.1.2/lib/active_record/integration.rb#32
  def collection_cache_versioning?; end

  # source://activerecord/7.1.2/lib/active_record/model_schema.rb#178
  def column_for_attribute(name, &block); end

  # source://activerecord/7.1.2/lib/active_record/core.rb#97
  def default_connection_handler; end

  # source://activerecord/7.1.2/lib/active_record/core.rb#97
  def default_connection_handler?; end

  # source://activerecord/7.1.2/lib/active_record/core.rb#99
  def default_role; end

  # source://activerecord/7.1.2/lib/active_record/core.rb#99
  def default_role?; end

  # source://activerecord/7.1.2/lib/active_record/scoping/default.rb#20
  def default_scope_override; end

  # source://activerecord/7.1.2/lib/active_record/scoping/default.rb#19
  def default_scopes; end

  # source://activerecord/7.1.2/lib/active_record/core.rb#101
  def default_shard; end

  # source://activerecord/7.1.2/lib/active_record/core.rb#101
  def default_shard?; end

  # source://activerecord/7.1.2/lib/active_record/enum.rb#167
  def defined_enums; end

  # source://activerecord/7.1.2/lib/active_record/enum.rb#167
  def defined_enums?; end

  # source://activerecord/7.1.2/lib/active_record/core.rb#47
  def destroy_association_async_batch_size; end

  # source://activerecord/7.1.2/lib/active_record/core.rb#37
  def destroy_association_async_job(&block); end

  # source://activerecord/7.1.2/lib/active_record/encryption/encryptable_record.rb#11
  def encrypted_attributes; end

  # source://activerecord/7.1.2/lib/active_record/encryption/encryptable_record.rb#11
  def encrypted_attributes=(_arg0); end

  # source://activerecord/7.1.2/lib/active_record/encryption/encryptable_record.rb#11
  def encrypted_attributes?; end

  # source://activemodel/7.1.2/lib/active_model/serializers/json.rb#15
  def include_root_in_json; end

  # source://activemodel/7.1.2/lib/active_model/serializers/json.rb#15
  def include_root_in_json?; end

  # source://activerecord/7.1.2/lib/active_record/locking/optimistic.rb#56
  def lock_optimistically; end

  # source://activerecord/7.1.2/lib/active_record/locking/optimistic.rb#56
  def lock_optimistically?; end

  # source://activerecord/7.1.2/lib/active_record/core.rb#22
  def logger; end

  # source://activerecord/7.1.2/lib/active_record/core.rb#22
  def logger?; end

  # source://activemodel/7.1.2/lib/active_model/naming.rb#255
  def model_name(&block); end

  # source://activerecord/7.1.2/lib/active_record/nested_attributes.rb#15
  def nested_attributes_options; end

  # source://activerecord/7.1.2/lib/active_record/nested_attributes.rb#15
  def nested_attributes_options?; end

  # source://activerecord/7.1.2/lib/active_record/normalization.rb#8
  def normalized_attributes; end

  # source://activerecord/7.1.2/lib/active_record/normalization.rb#8
  def normalized_attributes=(_arg0); end

  # source://activerecord/7.1.2/lib/active_record/normalization.rb#8
  def normalized_attributes?; end

  # source://activemodel/7.1.2/lib/active_model/conversion.rb#32
  def param_delimiter=(_arg0); end

  # source://activerecord/7.1.2/lib/active_record/attribute_methods/dirty.rb#50
  def partial_inserts; end

  # source://activerecord/7.1.2/lib/active_record/attribute_methods/dirty.rb#50
  def partial_inserts?; end

  # source://activerecord/7.1.2/lib/active_record/attribute_methods/dirty.rb#49
  def partial_updates; end

  # source://activerecord/7.1.2/lib/active_record/attribute_methods/dirty.rb#49
  def partial_updates?; end

  # source://activerecord/7.1.2/lib/active_record/model_schema.rb#163
  def pluralize_table_names; end

  # source://activerecord/7.1.2/lib/active_record/model_schema.rb#163
  def pluralize_table_names?; end

  # source://activerecord/7.1.2/lib/active_record/model_schema.rb#158
  def primary_key_prefix_type; end

  # source://activerecord/7.1.2/lib/active_record/model_schema.rb#158
  def primary_key_prefix_type?; end

  # source://activerecord/7.1.2/lib/active_record/timestamp.rb#47
  def record_timestamps; end

  # source://activerecord/7.1.2/lib/active_record/timestamp.rb#47
  def record_timestamps=(_arg0); end

  # source://activerecord/7.1.2/lib/active_record/timestamp.rb#47
  def record_timestamps?; end

  # source://activerecord/7.1.2/lib/active_record/signed_id.rb#13
  def signed_id_verifier_secret; end

  # source://activerecord/7.1.2/lib/active_record/signed_id.rb#13
  def signed_id_verifier_secret?; end

  # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#67
  def skip_time_zone_conversion_for_attributes; end

  # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#67
  def skip_time_zone_conversion_for_attributes?; end

  # source://activerecord/7.1.2/lib/active_record/inheritance.rb#43
  def store_full_class_name; end

  # source://activerecord/7.1.2/lib/active_record/inheritance.rb#43
  def store_full_class_name?; end

  # source://activerecord/7.1.2/lib/active_record/inheritance.rb#47
  def store_full_sti_class; end

  # source://activerecord/7.1.2/lib/active_record/inheritance.rb#47
  def store_full_sti_class?; end

  # source://activerecord/7.1.2/lib/active_record/model_schema.rb#159
  def table_name_prefix; end

  # source://activerecord/7.1.2/lib/active_record/model_schema.rb#159
  def table_name_prefix?; end

  # source://activerecord/7.1.2/lib/active_record/model_schema.rb#160
  def table_name_suffix; end

  # source://activerecord/7.1.2/lib/active_record/model_schema.rb#160
  def table_name_suffix?; end

  # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#66
  def time_zone_aware_attributes; end

  # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#66
  def time_zone_aware_attributes?; end

  # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#68
  def time_zone_aware_types; end

  # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#68
  def time_zone_aware_types?; end

  # source://delayed_job//lib/delayed/serialization/active_record.rb#12
  def to_yaml_properties; end

  # source://activerecord/7.1.2/lib/active_record/model_schema.rb#178
  def type_for_attribute(attr_name, &block); end

  # source://activemodel/7.1.2/lib/active_model/validations.rb#67
  def validation_context; end

  private

  # source://activemodel/7.1.2/lib/active_model/validations.rb#67
  def validation_context=(_arg0); end

  class << self
    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#70
    def __callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#70
    def __callbacks=(value); end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#70
    def __callbacks?; end

    # source://activerecord/7.1.2/lib/active_record/readonly_attributes.rb#11
    def _attr_readonly; end

    # source://activerecord/7.1.2/lib/active_record/readonly_attributes.rb#11
    def _attr_readonly=(value); end

    # source://activerecord/7.1.2/lib/active_record/readonly_attributes.rb#11
    def _attr_readonly?; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _before_commit_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _before_commit_callbacks=(value); end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _commit_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _commit_callbacks=(value); end

    # source://activerecord/7.1.2/lib/active_record/counter_cache.rb#9
    def _counter_cache_columns; end

    # source://activerecord/7.1.2/lib/active_record/counter_cache.rb#9
    def _counter_cache_columns=(value); end

    # source://activerecord/7.1.2/lib/active_record/counter_cache.rb#9
    def _counter_cache_columns?; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _create_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _create_callbacks=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#24
    def _destroy_association_async_job; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#24
    def _destroy_association_async_job=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#24
    def _destroy_association_async_job?; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _destroy_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _destroy_callbacks=(value); end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _find_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _find_callbacks=(value); end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _initialize_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _initialize_callbacks=(value); end

    # source://activerecord/7.1.2/lib/active_record/reflection.rb#11
    def _reflections; end

    # source://activerecord/7.1.2/lib/active_record/reflection.rb#11
    def _reflections=(value); end

    # source://activerecord/7.1.2/lib/active_record/reflection.rb#11
    def _reflections?; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _rollback_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _rollback_callbacks=(value); end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _save_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _save_callbacks=(value); end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _touch_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _touch_callbacks=(value); end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _update_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _update_callbacks=(value); end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _validate_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _validate_callbacks=(value); end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#955
    def _validation_callbacks; end

    # source://activesupport/7.1.2/lib/active_support/callbacks.rb#959
    def _validation_callbacks=(value); end

    # source://activemodel/7.1.2/lib/active_model/validations.rb#71
    def _validators; end

    # source://activemodel/7.1.2/lib/active_model/validations.rb#71
    def _validators=(value); end

    # source://activemodel/7.1.2/lib/active_model/validations.rb#71
    def _validators?; end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#144
    def after_create(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#144
    def after_destroy(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#144
    def after_find(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#144
    def after_initialize(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#144
    def after_save(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#144
    def after_touch(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#144
    def after_update(*args, **options, &block); end

    # source://activerecord/7.1.2/lib/active_record/reflection.rb#12
    def aggregate_reflections; end

    # source://activerecord/7.1.2/lib/active_record/reflection.rb#12
    def aggregate_reflections=(value); end

    # source://activerecord/7.1.2/lib/active_record/reflection.rb#12
    def aggregate_reflections?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#105
    def application_record_class?; end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#137
    def around_create(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#137
    def around_destroy(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#137
    def around_save(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#137
    def around_update(*args, **options, &block); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#125
    def asynchronous_queries_session; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#129
    def asynchronous_queries_tracker; end

    # source://activestorage/7.1.2/lib/active_storage/reflection.rb#53
    def attachment_reflections; end

    # source://activestorage/7.1.2/lib/active_storage/reflection.rb#53
    def attachment_reflections=(value); end

    # source://activestorage/7.1.2/lib/active_storage/reflection.rb#53
    def attachment_reflections?; end

    # source://activemodel/7.1.2/lib/active_model/attribute_methods.rb#72
    def attribute_aliases; end

    # source://activemodel/7.1.2/lib/active_model/attribute_methods.rb#72
    def attribute_aliases=(value); end

    # source://activemodel/7.1.2/lib/active_model/attribute_methods.rb#72
    def attribute_aliases?; end

    # source://activemodel/7.1.2/lib/active_model/attribute_methods.rb#73
    def attribute_method_patterns; end

    # source://activemodel/7.1.2/lib/active_model/attribute_methods.rb#73
    def attribute_method_patterns=(value); end

    # source://activemodel/7.1.2/lib/active_model/attribute_methods.rb#73
    def attribute_method_patterns?; end

    # source://activerecord/7.1.2/lib/active_record/attributes.rb#11
    def attributes_to_define_after_schema_loads; end

    # source://activerecord/7.1.2/lib/active_record/attributes.rb#11
    def attributes_to_define_after_schema_loads=(value); end

    # source://activerecord/7.1.2/lib/active_record/attributes.rb#11
    def attributes_to_define_after_schema_loads?; end

    # source://activerecord/7.1.2/lib/active_record/reflection.rb#13
    def automatic_scope_inversing; end

    # source://activerecord/7.1.2/lib/active_record/reflection.rb#13
    def automatic_scope_inversing=(value); end

    # source://activerecord/7.1.2/lib/active_record/reflection.rb#13
    def automatic_scope_inversing?; end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#130
    def before_create(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#130
    def before_destroy(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#130
    def before_save(*args, **options, &block); end

    # source://activemodel/7.1.2/lib/active_model/callbacks.rb#130
    def before_update(*args, **options, &block); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#89
    def belongs_to_required_by_default; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#89
    def belongs_to_required_by_default=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#89
    def belongs_to_required_by_default?; end

    # source://activerecord/7.1.2/lib/active_record/integration.rb#16
    def cache_timestamp_format; end

    # source://activerecord/7.1.2/lib/active_record/integration.rb#16
    def cache_timestamp_format=(value); end

    # source://activerecord/7.1.2/lib/active_record/integration.rb#16
    def cache_timestamp_format?; end

    # source://activerecord/7.1.2/lib/active_record/integration.rb#24
    def cache_versioning; end

    # source://activerecord/7.1.2/lib/active_record/integration.rb#24
    def cache_versioning=(value); end

    # source://activerecord/7.1.2/lib/active_record/integration.rb#24
    def cache_versioning?; end

    # source://activerecord/7.1.2/lib/active_record/integration.rb#32
    def collection_cache_versioning; end

    # source://activerecord/7.1.2/lib/active_record/integration.rb#32
    def collection_cache_versioning=(value); end

    # source://activerecord/7.1.2/lib/active_record/integration.rb#32
    def collection_cache_versioning?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#77
    def configurations; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#71
    def configurations=(config); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#189
    def connected_to_stack; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#203
    def connection_class; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#199
    def connection_class=(b); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#207
    def connection_class?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#211
    def connection_class_for_self; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#117
    def connection_handler; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#121
    def connection_handler=(handler); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#180
    def current_preventing_writes; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#143
    def current_role; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#161
    def current_shard; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/serialization.rb#20
    def default_column_serializer; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/serialization.rb#20
    def default_column_serializer=(value); end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/serialization.rb#20
    def default_column_serializer?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#97
    def default_connection_handler; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#97
    def default_connection_handler=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#97
    def default_connection_handler?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#99
    def default_role; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#99
    def default_role=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#99
    def default_role?; end

    # source://activerecord/7.1.2/lib/active_record/scoping/default.rb#20
    def default_scope_override; end

    # source://activerecord/7.1.2/lib/active_record/scoping/default.rb#20
    def default_scope_override=(value); end

    # source://activerecord/7.1.2/lib/active_record/scoping/default.rb#19
    def default_scopes; end

    # source://activerecord/7.1.2/lib/active_record/scoping/default.rb#19
    def default_scopes=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#101
    def default_shard; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#101
    def default_shard=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#101
    def default_shard?; end

    # source://activerecord/7.1.2/lib/active_record/enum.rb#167
    def defined_enums; end

    # source://activerecord/7.1.2/lib/active_record/enum.rb#167
    def defined_enums=(value); end

    # source://activerecord/7.1.2/lib/active_record/enum.rb#167
    def defined_enums?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#47
    def destroy_association_async_batch_size; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#47
    def destroy_association_async_batch_size=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#27
    def destroy_association_async_job; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#24
    def destroy_association_async_job=(value); end

    # source://activerecord/7.1.2/lib/active_record/encryption/encryptable_record.rb#11
    def encrypted_attributes; end

    # source://activerecord/7.1.2/lib/active_record/encryption/encryptable_record.rb#11
    def encrypted_attributes=(value); end

    # source://activerecord/7.1.2/lib/active_record/encryption/encryptable_record.rb#11
    def encrypted_attributes?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#87
    def enumerate_columns_in_select_statements; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#87
    def enumerate_columns_in_select_statements=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#87
    def enumerate_columns_in_select_statements?; end

    # source://activerecord/7.1.2/lib/active_record/token_for.rb#11
    def generated_token_verifier; end

    # source://activerecord/7.1.2/lib/active_record/token_for.rb#11
    def generated_token_verifier=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#93
    def has_many_inversing; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#93
    def has_many_inversing=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#93
    def has_many_inversing?; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#165
    def immutable_strings_by_default; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#165
    def immutable_strings_by_default=(value); end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#165
    def immutable_strings_by_default?; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#164
    def implicit_order_column; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#164
    def implicit_order_column=(value); end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#164
    def implicit_order_column?; end

    # source://activemodel/7.1.2/lib/active_model/serializers/json.rb#15
    def include_root_in_json; end

    # source://activemodel/7.1.2/lib/active_model/serializers/json.rb#15
    def include_root_in_json=(value); end

    # source://activemodel/7.1.2/lib/active_model/serializers/json.rb#15
    def include_root_in_json?; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#167
    def inheritance_column; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#321
    def inheritance_column=(value); end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#167
    def inheritance_column?; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#162
    def internal_metadata_table_name; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#162
    def internal_metadata_table_name=(value); end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#162
    def internal_metadata_table_name?; end

    # source://activerecord/7.1.2/lib/active_record/scoping/named.rb#174
    def jsonb_contains(*args, **_arg1); end

    # source://activerecord/7.1.2/lib/active_record/scoping/named.rb#174
    def jsonb_excludes(*args, **_arg1); end

    # source://activerecord/7.1.2/lib/active_record/scoping/named.rb#174
    def jsonb_number_where(*args, **_arg1); end

    # source://activerecord/7.1.2/lib/active_record/scoping/named.rb#174
    def jsonb_number_where_not(*args, **_arg1); end

    # source://activerecord/7.1.2/lib/active_record/scoping/named.rb#174
    def jsonb_order(*args, **_arg1); end

    # source://activerecord/7.1.2/lib/active_record/scoping/named.rb#174
    def jsonb_time_where(*args, **_arg1); end

    # source://activerecord/7.1.2/lib/active_record/scoping/named.rb#174
    def jsonb_time_where_not(*args, **_arg1); end

    # source://activerecord/7.1.2/lib/active_record/scoping/named.rb#174
    def jsonb_where(*args, **_arg1); end

    # source://activerecord/7.1.2/lib/active_record/scoping/named.rb#174
    def jsonb_where_not(*args, **_arg1); end

    # source://activerecord/7.1.2/lib/active_record/store.rb#101
    def local_stored_attributes; end

    # source://activerecord/7.1.2/lib/active_record/store.rb#101
    def local_stored_attributes=(_arg0); end

    # source://activerecord/7.1.2/lib/active_record/locking/optimistic.rb#56
    def lock_optimistically; end

    # source://activerecord/7.1.2/lib/active_record/locking/optimistic.rb#56
    def lock_optimistically=(value); end

    # source://activerecord/7.1.2/lib/active_record/locking/optimistic.rb#56
    def lock_optimistically?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#22
    def logger; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#22
    def logger=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#22
    def logger?; end

    # source://activerecord/7.1.2/lib/active_record/nested_attributes.rb#15
    def nested_attributes_options; end

    # source://activerecord/7.1.2/lib/active_record/nested_attributes.rb#15
    def nested_attributes_options=(value); end

    # source://activerecord/7.1.2/lib/active_record/nested_attributes.rb#15
    def nested_attributes_options?; end

    # source://activerecord/7.1.2/lib/active_record/normalization.rb#8
    def normalized_attributes; end

    # source://activerecord/7.1.2/lib/active_record/normalization.rb#8
    def normalized_attributes=(value); end

    # source://activerecord/7.1.2/lib/active_record/normalization.rb#8
    def normalized_attributes?; end

    # source://activemodel/7.1.2/lib/active_model/conversion.rb#32
    def param_delimiter; end

    # source://activemodel/7.1.2/lib/active_model/conversion.rb#32
    def param_delimiter=(value); end

    # source://activemodel/7.1.2/lib/active_model/conversion.rb#32
    def param_delimiter?; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/dirty.rb#50
    def partial_inserts; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/dirty.rb#50
    def partial_inserts=(value); end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/dirty.rb#50
    def partial_inserts?; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/dirty.rb#49
    def partial_updates; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/dirty.rb#49
    def partial_updates=(value); end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/dirty.rb#49
    def partial_updates?; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#163
    def pluralize_table_names; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#163
    def pluralize_table_names=(value); end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#163
    def pluralize_table_names?; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#158
    def primary_key_prefix_type; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#158
    def primary_key_prefix_type=(value); end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#158
    def primary_key_prefix_type?; end

    # source://activerecord/7.1.2/lib/active_record/timestamp.rb#47
    def record_timestamps; end

    # source://activerecord/7.1.2/lib/active_record/timestamp.rb#47
    def record_timestamps=(value); end

    # source://activerecord/7.1.2/lib/active_record/timestamp.rb#47
    def record_timestamps?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#95
    def run_commit_callbacks_on_first_saved_instances_in_transaction; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#95
    def run_commit_callbacks_on_first_saved_instances_in_transaction=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#95
    def run_commit_callbacks_on_first_saved_instances_in_transaction?; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#161
    def schema_migrations_table_name; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#161
    def schema_migrations_table_name=(value); end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#161
    def schema_migrations_table_name?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#103
    def shard_selector; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#103
    def shard_selector=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#103
    def shard_selector?; end

    # source://activerecord/7.1.2/lib/active_record/signed_id.rb#13
    def signed_id_verifier_secret; end

    # source://activerecord/7.1.2/lib/active_record/signed_id.rb#13
    def signed_id_verifier_secret=(value); end

    # source://activerecord/7.1.2/lib/active_record/signed_id.rb#13
    def signed_id_verifier_secret?; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#67
    def skip_time_zone_conversion_for_attributes; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#67
    def skip_time_zone_conversion_for_attributes=(value); end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#67
    def skip_time_zone_conversion_for_attributes?; end

    # source://activerecord/7.1.2/lib/active_record/inheritance.rb#43
    def store_full_class_name; end

    # source://activerecord/7.1.2/lib/active_record/inheritance.rb#43
    def store_full_class_name=(value); end

    # source://activerecord/7.1.2/lib/active_record/inheritance.rb#43
    def store_full_class_name?; end

    # source://activerecord/7.1.2/lib/active_record/inheritance.rb#47
    def store_full_sti_class; end

    # source://activerecord/7.1.2/lib/active_record/inheritance.rb#47
    def store_full_sti_class=(value); end

    # source://activerecord/7.1.2/lib/active_record/inheritance.rb#47
    def store_full_sti_class?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#91
    def strict_loading_by_default; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#91
    def strict_loading_by_default=(value); end

    # source://activerecord/7.1.2/lib/active_record/core.rb#91
    def strict_loading_by_default?; end

    # source://activerecord/7.1.2/lib/active_record/core.rb#226
    def strict_loading_violation!(owner:, reflection:); end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#159
    def table_name_prefix; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#159
    def table_name_prefix=(value); end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#159
    def table_name_prefix?; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#160
    def table_name_suffix; end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#160
    def table_name_suffix=(value); end

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#160
    def table_name_suffix?; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#66
    def time_zone_aware_attributes; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#66
    def time_zone_aware_attributes=(value); end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#66
    def time_zone_aware_attributes?; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#68
    def time_zone_aware_types; end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#68
    def time_zone_aware_types=(value); end

    # source://activerecord/7.1.2/lib/active_record/attribute_methods/time_zone_conversion.rb#68
    def time_zone_aware_types?; end

    # source://activerecord/7.1.2/lib/active_record/token_for.rb#10
    def token_definitions; end

    # source://activerecord/7.1.2/lib/active_record/token_for.rb#10
    def token_definitions=(value); end

    # source://delayed_job//lib/delayed/serialization/active_record.rb#6
    def yaml_new(klass, _tag, val); end

    private

    # source://activerecord/7.1.2/lib/active_record/model_schema.rb#167
    def _inheritance_column=(value); end
  end
end

# source://delayed_job//lib/delayed/compatibility.rb#3
module Delayed; end

# source://delayed_job//lib/delayed/backend/base.rb#2
module Delayed::Backend; end

# source://delayed_job//lib/delayed/backend/base.rb#3
module Delayed::Backend::Base
  mixes_in_class_methods ::Delayed::Backend::Base::ClassMethods

  # @return [Boolean]
  #
  # source://delayed_job//lib/delayed/backend/base.rb#129
  def destroy_failed_jobs?; end

  # Returns the value of attribute error.
  #
  # source://delayed_job//lib/delayed/backend/base.rb#47
  def error; end

  # source://delayed_job//lib/delayed/backend/base.rb#48
  def error=(error); end

  # source://delayed_job//lib/delayed/backend/base.rb#135
  def fail!; end

  # @return [Boolean]
  #
  # source://delayed_job//lib/delayed/backend/base.rb#53
  def failed; end

  # @return [Boolean]
  #
  # source://delayed_job//lib/delayed/backend/base.rb#53
  def failed?; end

  # source://delayed_job//lib/delayed/backend/base.rb#98
  def hook(name, *args); end

  # source://delayed_job//lib/delayed/backend/base.rb#77
  def invoke_job; end

  # source://delayed_job//lib/delayed/backend/base.rb#114
  def max_attempts; end

  # source://delayed_job//lib/delayed/backend/base.rb#118
  def max_run_time; end

  # source://delayed_job//lib/delayed/backend/base.rb#60
  def name; end

  # source://delayed_job//lib/delayed/backend/base.rb#71
  def payload_object; end

  # source://delayed_job//lib/delayed/backend/base.rb#66
  def payload_object=(object); end

  # source://delayed_job//lib/delayed/backend/base.rb#106
  def reschedule_at; end

  # Unlock this job (note: not saved to DB)
  #
  # source://delayed_job//lib/delayed/backend/base.rb#93
  def unlock; end

  protected

  # Call during reload operation to clear out internal state
  #
  # source://delayed_job//lib/delayed/backend/base.rb#147
  def reset; end

  # source://delayed_job//lib/delayed/backend/base.rb#142
  def set_default_run_at; end

  class << self
    # @private
    #
    # source://delayed_job//lib/delayed/backend/base.rb#4
    def included(base); end
  end
end

# source://delayed_job//lib/delayed/backend/base.rb#8
module Delayed::Backend::Base::ClassMethods
  # Hook method that is called after a new worker is forked
  #
  # source://delayed_job//lib/delayed/backend/base.rb#39
  def after_fork; end

  # Hook method that is called before a new worker is forked
  #
  # source://delayed_job//lib/delayed/backend/base.rb#36
  def before_fork; end

  # Add a job to the queue
  #
  # source://delayed_job//lib/delayed/backend/base.rb#10
  def enqueue(*args); end

  # source://delayed_job//lib/delayed/backend/base.rb#15
  def enqueue_job(options); end

  # Allow the backend to attempt recovery from reserve errors
  #
  # source://delayed_job//lib/delayed/backend/base.rb#33
  def recover_from(_error); end

  # source://delayed_job//lib/delayed/backend/base.rb#24
  def reserve(worker, max_run_time = T.unsafe(nil)); end

  # source://delayed_job//lib/delayed/backend/base.rb#41
  def work_off(num = T.unsafe(nil)); end
end

# source://delayed_job//lib/delayed/backend/base.rb#58
Delayed::Backend::Base::ParseObjectFromYaml = T.let(T.unsafe(nil), Regexp)

# source://delayed_job//lib/delayed/backend/job_preparer.rb#3
class Delayed::Backend::JobPreparer
  # @return [JobPreparer] a new instance of JobPreparer
  #
  # source://delayed_job//lib/delayed/backend/job_preparer.rb#6
  def initialize(*args); end

  # Returns the value of attribute args.
  #
  # source://delayed_job//lib/delayed/backend/job_preparer.rb#4
  def args; end

  # Returns the value of attribute options.
  #
  # source://delayed_job//lib/delayed/backend/job_preparer.rb#4
  def options; end

  # source://delayed_job//lib/delayed/backend/job_preparer.rb#11
  def prepare; end

  private

  # source://delayed_job//lib/delayed/backend/job_preparer.rb#38
  def handle_deprecation; end

  # source://delayed_job//lib/delayed/backend/job_preparer.rb#21
  def set_payload; end

  # source://delayed_job//lib/delayed/backend/job_preparer.rb#33
  def set_priority; end

  # source://delayed_job//lib/delayed/backend/job_preparer.rb#25
  def set_queue_name; end
end

# source://delayed_job//lib/delayed/lifecycle.rb#55
class Delayed::Callback
  # @return [Callback] a new instance of Callback
  #
  # source://delayed_job//lib/delayed/lifecycle.rb#56
  def initialize; end

  # source://delayed_job//lib/delayed/lifecycle.rb#71
  def add(type, &callback); end

  # source://delayed_job//lib/delayed/lifecycle.rb#64
  def execute(*args, &block); end
end

# source://delayed_job//lib/delayed/compatibility.rb#4
module Delayed::Compatibility
  class << self
    # source://delayed_job//lib/delayed/compatibility.rb#8
    def executable_prefix; end

    # source://delayed_job//lib/delayed/compatibility.rb#12
    def proxy_object_class; end
  end
end

# source://delayed_job//lib/delayed/performable_mailer.rb#11
module Delayed::DelayMail
  # source://delayed_job//lib/delayed/performable_mailer.rb#12
  def delay(options = T.unsafe(nil)); end
end

# source://delayed_job//lib/delayed/message_sending.rb#2
class Delayed::DelayProxy < ::ActiveSupport::ProxyObject
  # @return [DelayProxy] a new instance of DelayProxy
  #
  # source://delayed_job//lib/delayed/message_sending.rb#3
  def initialize(payload_class, target, options); end

  # source://delayed_job//lib/delayed/message_sending.rb#10
  def method_missing(method, *args); end
end

# source://delayed_job//lib/delayed/deserialization_error.rb#2
class Delayed::DeserializationError < ::StandardError; end

# source://delayed_job//lib/delayed/exceptions.rb#11
class Delayed::FatalBackendError < ::RuntimeError; end

# source://delayed_job//lib/delayed/lifecycle.rb#2
class Delayed::InvalidCallback < ::RuntimeError; end

# source://delayed_job//lib/delayed/worker.rb#72
Delayed::Job = Delayed::Backend::ActiveRecord::Job

# source://delayed_job//lib/delayed/lifecycle.rb#4
class Delayed::Lifecycle
  # @return [Lifecycle] a new instance of Lifecycle
  #
  # source://delayed_job//lib/delayed/lifecycle.rb#15
  def initialize; end

  # source://delayed_job//lib/delayed/lifecycle.rb#25
  def after(event, &block); end

  # source://delayed_job//lib/delayed/lifecycle.rb#29
  def around(event, &block); end

  # source://delayed_job//lib/delayed/lifecycle.rb#21
  def before(event, &block); end

  # source://delayed_job//lib/delayed/lifecycle.rb#33
  def run_callbacks(event, *args, &block); end

  private

  # source://delayed_job//lib/delayed/lifecycle.rb#45
  def add(type, event, &block); end

  # @raise [InvalidCallback]
  #
  # source://delayed_job//lib/delayed/lifecycle.rb#50
  def missing_callback(event); end
end

# source://delayed_job//lib/delayed/lifecycle.rb#5
Delayed::Lifecycle::EVENTS = T.let(T.unsafe(nil), Hash)

# source://delayed_job//lib/delayed/message_sending.rb#16
module Delayed::MessageSending
  # source://delayed_job//lib/delayed/message_sending.rb#17
  def __delay__(options = T.unsafe(nil)); end

  # source://delayed_job//lib/delayed/message_sending.rb#17
  def delay(options = T.unsafe(nil)); end

  # source://delayed_job//lib/delayed/message_sending.rb#27
  def send_at(time, method, *args); end

  # source://delayed_job//lib/delayed/message_sending.rb#22
  def send_later(method, *args); end
end

# source://delayed_job//lib/delayed/message_sending.rb#33
module Delayed::MessageSendingClassMethods
  # source://delayed_job//lib/delayed/message_sending.rb#34
  def handle_asynchronously(method, opts = T.unsafe(nil)); end
end

# source://delayed_job//lib/delayed/performable_mailer.rb#4
class Delayed::PerformableMailer < ::Delayed::PerformableMethod
  # source://delayed_job//lib/delayed/performable_mailer.rb#5
  def perform; end
end

# source://delayed_job//lib/delayed/performable_method.rb#2
class Delayed::PerformableMethod
  # @raise [NoMethodError]
  # @return [PerformableMethod] a new instance of PerformableMethod
  #
  # source://delayed_job//lib/delayed/performable_method.rb#5
  def initialize(object, method_name, args); end

  # Returns the value of attribute args.
  #
  # source://delayed_job//lib/delayed/performable_method.rb#3
  def args; end

  # Sets the attribute args
  #
  # @param value the value to set the attribute args to.
  #
  # source://delayed_job//lib/delayed/performable_method.rb#3
  def args=(_arg0); end

  # source://delayed_job//lib/delayed/performable_method.rb#17
  def display_name; end

  # serialize to YAML
  #
  # source://delayed_job//lib/delayed/psych_ext.rb#4
  def encode_with(coder); end

  # source://delayed_job//lib/delayed/performable_method.rb#29
  def method(sym); end

  # source://delayed_job//lib/delayed/performable_method.rb#34
  def method_missing(symbol, *args); end

  # Returns the value of attribute method_name.
  #
  # source://delayed_job//lib/delayed/performable_method.rb#3
  def method_name; end

  # Sets the attribute method_name
  #
  # @param value the value to set the attribute method_name to.
  #
  # source://delayed_job//lib/delayed/performable_method.rb#3
  def method_name=(_arg0); end

  # Returns the value of attribute object.
  #
  # source://delayed_job//lib/delayed/performable_method.rb#3
  def object; end

  # Sets the attribute object
  #
  # @param value the value to set the attribute object to.
  #
  # source://delayed_job//lib/delayed/performable_method.rb#3
  def object=(_arg0); end

  # source://delayed_job//lib/delayed/performable_method.rb#25
  def perform; end

  # @return [Boolean]
  #
  # source://delayed_job//lib/delayed/performable_method.rb#39
  def respond_to?(symbol, include_private = T.unsafe(nil)); end
end

# source://delayed_job//lib/delayed/plugin.rb#4
class Delayed::Plugin
  # @return [Plugin] a new instance of Plugin
  #
  # source://delayed_job//lib/delayed/plugin.rb#11
  def initialize; end

  # source://delayed_job//lib/delayed/plugin.rb#5
  def callback_block; end

  # source://delayed_job//lib/delayed/plugin.rb#5
  def callback_block=(_arg0); end

  # source://delayed_job//lib/delayed/plugin.rb#5
  def callback_block?; end

  class << self
    # source://delayed_job//lib/delayed/plugin.rb#5
    def callback_block; end

    # source://delayed_job//lib/delayed/plugin.rb#5
    def callback_block=(value); end

    # source://delayed_job//lib/delayed/plugin.rb#5
    def callback_block?; end

    # source://delayed_job//lib/delayed/plugin.rb#7
    def callbacks(&block); end
  end
end

# source://delayed_job//lib/delayed/plugins/clear_locks.rb#2
module Delayed::Plugins; end

# source://delayed_job//lib/delayed/plugins/clear_locks.rb#3
class Delayed::Plugins::ClearLocks < ::Delayed::Plugin
  class << self
    # source://delayed_job//lib/delayed/plugin.rb#5
    def callback_block; end
  end
end

# source://delayed_job//lib/delayed/psych_ext.rb#22
module Delayed::PsychExt; end

# source://delayed_job//lib/delayed/psych_ext.rb#23
class Delayed::PsychExt::ToRuby < ::Psych::Visitors::ToRuby
  # defined? is triggering something really messed up in
  # jruby causing both the if AND else clauses to execute,
  # however if the check is run here, everything is fine
  #
  # source://delayed_job//lib/delayed/psych_ext.rb#90
  def jruby_is_seriously_borked; end

  # source://delayed_job//lib/delayed/psych_ext.rb#94
  def resolve_class(klass_name); end

  # source://delayed_job//lib/delayed/psych_ext.rb#30
  def visit_Psych_Nodes_Mapping(object); end
end

# source://delayed_job//lib/delayed/railtie.rb#5
class Delayed::Railtie < ::Rails::Railtie; end

# source://delayed_job//lib/delayed/worker.rb#12
class Delayed::Worker
  # @return [Worker] a new instance of Worker
  #
  # source://delayed_job//lib/delayed/worker.rb#131
  def initialize(options = T.unsafe(nil)); end

  # source://delayed_job//lib/delayed/worker.rb#31
  def backend; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def default_log_level; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def default_log_level=(val); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def default_priority; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def default_priority=(val); end

  # source://delayed_job//lib/delayed/worker.rb#29
  def default_queue_name; end

  # source://delayed_job//lib/delayed/worker.rb#29
  def default_queue_name=(val); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def delay_jobs; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def delay_jobs=(val); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def destroy_failed_jobs; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def destroy_failed_jobs=(val); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def exit_on_complete; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def exit_on_complete=(val); end

  # source://delayed_job//lib/delayed/worker.rb#260
  def failed(job); end

  # source://delayed_job//lib/delayed/worker.rb#273
  def job_say(job, text, level = T.unsafe(nil)); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def logger; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def logger=(val); end

  # source://delayed_job//lib/delayed/worker.rb#289
  def max_attempts(job); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def max_attempts=(val); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def max_priority; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def max_priority=(val); end

  # source://delayed_job//lib/delayed/worker.rb#293
  def max_run_time(job); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def max_run_time=(val); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def min_priority; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def min_priority=(val); end

  # Every worker has a unique name which by default is the pid of the process. There are some
  # advantages to overriding this with something which survives worker restarts:  Workers can
  # safely resume working on tasks which are locked by themselves. The worker will assume that
  # it crashed before.
  #
  # source://delayed_job//lib/delayed/worker.rb#148
  def name; end

  # Sets the name of the worker.
  # Setting the name to nil will reset the default worker name
  #
  # source://delayed_job//lib/delayed/worker.rb#155
  def name=(_arg0); end

  # name_prefix is ignored if name is set directly
  #
  # source://delayed_job//lib/delayed/worker.rb#34
  def name_prefix; end

  # name_prefix is ignored if name is set directly
  #
  # source://delayed_job//lib/delayed/worker.rb#34
  def name_prefix=(_arg0); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def plugins; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def plugins=(val); end

  # source://delayed_job//lib/delayed/worker.rb#31
  def queue_attributes; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def queues; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def queues=(val); end

  # source://delayed_job//lib/delayed/worker.rb#62
  def raise_signal_exceptions; end

  # source://delayed_job//lib/delayed/worker.rb#62
  def raise_signal_exceptions=(val); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def read_ahead; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def read_ahead=(val); end

  # Reschedule the job in the future (when a job fails).
  # Uses an exponential scale depending on the number of failed attempts.
  #
  # source://delayed_job//lib/delayed/worker.rb#248
  def reschedule(job, time = T.unsafe(nil)); end

  # source://delayed_job//lib/delayed/worker.rb#228
  def run(job); end

  # source://delayed_job//lib/delayed/worker.rb#278
  def say(text, level = T.unsafe(nil)); end

  # source://delayed_job//lib/delayed/worker.rb#23
  def sleep_delay; end

  # source://delayed_job//lib/delayed/worker.rb#23
  def sleep_delay=(val); end

  # source://delayed_job//lib/delayed/worker.rb#157
  def start; end

  # source://delayed_job//lib/delayed/worker.rb#199
  def stop; end

  # @return [Boolean]
  #
  # source://delayed_job//lib/delayed/worker.rb#203
  def stop?; end

  # Do num jobs and return stats on success/failure.
  # Exit early if interrupted.
  #
  # source://delayed_job//lib/delayed/worker.rb#209
  def work_off(num = T.unsafe(nil)); end

  protected

  # source://delayed_job//lib/delayed/worker.rb#303
  def handle_failed_job(job, error); end

  # source://delayed_job//lib/delayed/worker.rb#328
  def reload!; end

  # Run the next job we can get an exclusive lock on.
  # If no jobs are left we return nil
  #
  # source://delayed_job//lib/delayed/worker.rb#311
  def reserve_and_run_one_job; end

  # source://delayed_job//lib/delayed/worker.rb#316
  def reserve_job; end

  # source://delayed_job//lib/delayed/worker.rb#299
  def say_queue(queue); end

  class << self
    # source://delayed_job//lib/delayed/worker.rb#95
    def after_fork; end

    # source://delayed_job//lib/delayed/worker.rb#31
    def backend; end

    # source://delayed_job//lib/delayed/worker.rb#65
    def backend=(backend); end

    # source://delayed_job//lib/delayed/worker.rb#84
    def before_fork; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def default_log_level; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def default_log_level=(val); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def default_priority; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def default_priority=(val); end

    # source://delayed_job//lib/delayed/worker.rb#29
    def default_queue_name; end

    # source://delayed_job//lib/delayed/worker.rb#29
    def default_queue_name=(val); end

    # @return [Boolean]
    #
    # source://delayed_job//lib/delayed/worker.rb#123
    def delay_job?(job); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def delay_jobs; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def delay_jobs=(val); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def destroy_failed_jobs; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def destroy_failed_jobs=(val); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def exit_on_complete; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def exit_on_complete=(val); end

    # source://delayed_job//lib/delayed/worker.rb#80
    def guess_backend; end

    # source://delayed_job//lib/delayed/worker.rb#107
    def lifecycle; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def logger; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def logger=(val); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def max_attempts; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def max_attempts=(val); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def max_priority; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def max_priority=(val); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def max_run_time; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def max_run_time=(val); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def min_priority; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def min_priority=(val); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def plugins; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def plugins=(val); end

    # source://delayed_job//lib/delayed/worker.rb#31
    def queue_attributes; end

    # source://delayed_job//lib/delayed/worker.rb#76
    def queue_attributes=(val); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def queues; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def queues=(val); end

    # source://delayed_job//lib/delayed/worker.rb#62
    def raise_signal_exceptions; end

    # source://delayed_job//lib/delayed/worker.rb#62
    def raise_signal_exceptions=(val); end

    # source://delayed_job//lib/delayed/worker.rb#23
    def read_ahead; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def read_ahead=(val); end

    # @return [Boolean]
    #
    # source://delayed_job//lib/delayed/worker.rb#119
    def reload_app?; end

    # source://delayed_job//lib/delayed/worker.rb#36
    def reset; end

    # source://delayed_job//lib/delayed/worker.rb#114
    def setup_lifecycle; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def sleep_delay; end

    # source://delayed_job//lib/delayed/worker.rb#23
    def sleep_delay=(val); end
  end
end

# source://delayed_job//lib/delayed/worker.rb#17
Delayed::Worker::DEFAULT_DEFAULT_PRIORITY = T.let(T.unsafe(nil), Integer)

# source://delayed_job//lib/delayed/worker.rb#18
Delayed::Worker::DEFAULT_DELAY_JOBS = T.let(T.unsafe(nil), TrueClass)

# source://delayed_job//lib/delayed/worker.rb#13
Delayed::Worker::DEFAULT_LOG_LEVEL = T.let(T.unsafe(nil), String)

# source://delayed_job//lib/delayed/worker.rb#15
Delayed::Worker::DEFAULT_MAX_ATTEMPTS = T.let(T.unsafe(nil), Integer)

# source://delayed_job//lib/delayed/worker.rb#16
Delayed::Worker::DEFAULT_MAX_RUN_TIME = T.let(T.unsafe(nil), ActiveSupport::Duration)

# source://delayed_job//lib/delayed/worker.rb#19
Delayed::Worker::DEFAULT_QUEUES = T.let(T.unsafe(nil), Array)

# source://delayed_job//lib/delayed/worker.rb#20
Delayed::Worker::DEFAULT_QUEUE_ATTRIBUTES = T.let(T.unsafe(nil), ActiveSupport::HashWithIndifferentAccess)

# source://delayed_job//lib/delayed/worker.rb#21
Delayed::Worker::DEFAULT_READ_AHEAD = T.let(T.unsafe(nil), Integer)

# source://delayed_job//lib/delayed/worker.rb#14
Delayed::Worker::DEFAULT_SLEEP_DELAY = T.let(T.unsafe(nil), Integer)

# source://delayed_job//lib/delayed/exceptions.rb#4
class Delayed::WorkerTimeout < ::Timeout::Error
  # source://delayed_job//lib/delayed/exceptions.rb#5
  def message; end
end

class Module
  include ::Delayed::MessageSendingClassMethods
end

class Object < ::BasicObject
  include ::Kernel
  include ::PP::ObjectMixin
  include ::Delayed::MessageSending
end

# source://delayed_job//lib/delayed/psych_ext.rb#14
module Psych
  extend ::Bootsnap::CompileCache::YAML::Psych4::Patch

  class << self
    # source://delayed_job//lib/delayed/psych_ext.rb#15
    def load_dj(yaml); end
  end
end
